...
exec {'drop-existing-project-user':
    ...
    command => 'psql -d postgres -c "DROP USER IF EXISTS acts_open_house"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'create-project-user':
    ...
    command => 'psql -d postgres -c "CREATE USER acts_open_house WITH PASSWORD \'acts_open_house\'"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'drop-project-db':
    ...
    command => 'psql -d postgres -c "DROP DATABASE IF EXISTS acts_open_house"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'create-project-db':
    ...
    command => 'psql -d postgres -c "CREATE DATABASE acts_open_house WITH OWNER acts_open_house"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
# Create a symlink from ~/acts_open_house to /vagrant as a convenience for the developer
file {'/home/vagrant/acts_open_house':
    ensure => link,
    target => '/vagrant',
}

# Create a virtualenv for acts_open_house
exec {'create-virtualenv':
    provider => 'shell',
    user => 'vagrant',
    group => 'vagrant',
    require => [ Package['virtualenvwrapper'], File['/home/vagrant/acts_open_house'], Exec['known_hosts'], Exec['drop-project-db']],
    environment => ["HOME=/home/vagrant","WORKON_HOME=/home/vagrant/.virtualenvs"],
    command => '/vagrant/vagrant/venv_bootstrap.sh',
    creates => '/home/vagrant/.virtualenvs/acts_open_house',
}

# set the DJANGO_SETTINGS_MODULE environment variable
file_line {'add DJANGO_SETTINGS_MODULE env to postactivate':
    ensure => present,
    line => 'export DJANGO_SETTINGS_MODULE=acts_open_house.settings.local',
    path => '/home/vagrant/.virtualenvs/acts_open_house/bin/postactivate',
...
file_line {'add DJANGO_SETTINGS_MODULE env to postdeactivate':
    ...
    path => '/home/vagrant/.virtualenvs/acts_open_house/bin/postdeactivate',
    require => Exec['create-virtualenv'],
}
